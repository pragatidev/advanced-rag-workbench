"""Lab CLI. python -m ragbench ask is a test harness around run_ask, not the product."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "serve":
        from ragbench.serve import serve_forever
    ...
    if args.cmd == "inspect":
        ...
        if loaded is None:
            sys.stderr.write(f"no index at store/{args.name}/. Run: python -m ragbench index {args.name}\n")
            return 1
